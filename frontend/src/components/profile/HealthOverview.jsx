...
function HealthOverview({ student, healthScore }) {
  const healthStats = [
    {
      title: "Heart Rate",
      value: student.vitals?.heartRate || "Not recorded",
      status: "Normal",
      icon: HeartPulse,
      bg: "bg-red-100",
      text: "text-red-600",
    ...
    {
      title: "Temperature",
      value: student.vitals?.temperature || "Not recorded",
      status: "Stable",
      icon: Thermometer,
      bg: "bg-orange-100",
  ...
  return (
    <div>
      <h2 className="mb-6 text-2xl font-bold text-slate-900">Health Overview</h2>
      <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
        {healthStats.map((item) => {
          const Icon = item.icon;
          return (
            <GlassCard key={item.title} className="p-6 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl">
              <div className={`flex h-14 w-14 items-center justify-center rounded-2xl ${item.bg}`}>
                <Icon size={28} className={item.text} />
              </div>
              <h3 className="mt-5 text-lg font-semibold text-slate-700">{item.title}</h3>
