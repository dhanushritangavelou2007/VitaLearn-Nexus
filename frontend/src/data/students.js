...
const FUTURE_VACCINATION_EVENTS = [
  { name: "Seasonal Flu Booster", monthOffset: 1, note: "Annual flu prevention dose." },
  { name: "Hepatitis A Booster", monthOffset: 4, note: "Follow-up vaccine for immunity." },
  { name: "Tdap Booster", monthOffset: 7, note: "Tetanus, diphtheria and pertussis follow-up." },
];

students.forEach((student) => {
  if (student.healthScore == null) {
   student.healthScore = getHealthScore(student);
  }

  if (!student.vaccinationSchedule) {
   const startDay = 3 + ((student.id * 4) % 21);
   student.vaccinationSchedule = FUTURE_VACCINATION_EVENTS.map((event, index) => {
     const date = new Date(2024, 8 + event.monthOffset + index, ((startDay + index * 5) % 28) + 1);
     return {
       name: event.name,
...
export function calculateHealthScore(student) {
  let score = 100;
  const condition = student?.healthCondition;
  if (condition === "Fever") {
    score -= 15;
  } else if (condition === "Asthma Flare") {
    score -= 20;
  ...
  if (student?.attendance != null) {
    if (typeof student.attendance === "number") {
      ...
      const parsed = parseFloat(String(student.attendance).replace("%", ""));
...
export function getHealthScore(student) {
  return calculateHealthScore(student);
}
