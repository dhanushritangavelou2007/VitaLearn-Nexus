...
import students, { getRiskLabel, getRiskStyle, getStudentAvatar } from "../../data/students";
...
function Students() {
  ...
  student.class.toLowerCase().includes(normalizedSearch) ||
  student.bloodGroup.toLowerCase().includes(normalizedSearch);
  ...
  <div className="mt-3 flex flex-wrap gap-2">
    ...
    <span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-semibold text-emerald-700">
      Healthy: {students.filter(s => s.risk === "healthy").length}
    </span>

    <span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-semibold text-amber-700">
      Review: {students.filter(s => s.risk === "review").length}
    </span>
                ...
                {filteredStudents.length === 0 ? (
                  ...
                  filteredStudents.map((student) => {
                    ...
                    return (
                     <tr
  key={student.id}
  className="group transition-all duration-300 hover:bg-blue-50/40 hover:shadow-sm"
>
